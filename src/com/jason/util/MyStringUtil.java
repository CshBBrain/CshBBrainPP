...
public class MyStringUtil {
	 ...
	 * <li>修改日期：
	 */
	public static Date long2date(long datelong, String format) {
		if(isBlank(format)){
...
//		String a ="akdslkafk,alsdlksfk,2dadaf,849949,84399,,,";
    ...
    public static HashMap<String,String> parseKeyValue(String msg){    	
        ...
        int pathEndPos = values.indexOf('?');
        if (pathEndPos >= 0){
            requestData.put(HttpConstants.REQUEST, values.substring(0, pathEndPos));
        }
        
        System.out.println(values + "  : " + requestData.get(HttpConstants.REQUEST));
        
        Matcher m = PARAM_PATTERN.matcher(values.substring(pathEndPos + 1));
        int pos = 0;
        while (m.find(pos)) {
            pos = m.end();            
